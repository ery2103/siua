...
if (utility::isMemberLogin()) {
    $header_info .= '<div id="memberLoginInfo">'.__('You are currently Logged on as staff').': <strong>'.$_SESSION['m_name'].' (<em>'.$_SESSION['m_email'].'</em>)</strong> <a id="memberLogout" href="index.php?p=member&logout=1">['.__('LOGOUT').']</a></div>';
}
...
if (isset($_GET['p'])) {
    ...
    if (!isset($_GET['p'])) {
        if ((!isset($_GET['keywords'])) AND (!isset($_GET['page'])) AND (!isset($_GET['perihal'])) AND (!isset($_GET['ruas'])) AND (!isset($_GET['pengirim'])) AND (!isset($_GET['jenis'])) AND (!isset($_GET['lokasi'])) AND (!isset($_GET['tahun']))) {
            // get content data from database
            include LIB_DIR.'content.inc.php';
            $content = new content();
